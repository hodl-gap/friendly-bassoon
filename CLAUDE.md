...
- [x] **Tag output tracks as interpretation vs prediction** — Superseded by two-mode output refactor: retrospective mode produces causal decomposition (interpretation), prospective mode produces scenario analysis with structured predictions. The output mode IS the track type. Predictions are stored in `data/predictions.json` via `prediction_store.py`.
